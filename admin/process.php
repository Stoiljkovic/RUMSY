...
class Process
{
   ...
   function procSubmitComment(){
      ...
      if(($_POST['killbill'] != "")) {header("Location: ".$session->referrer."?comment=2");} else {
      $retval = $session->submitComment($user, $url, $msg , $_POST['status'], $ctime);
      }
      /* Account edit successful */
      if($retval){
         header("Location: ".$session->referrer."?comment=1");
      }
      /* Error found with form */
      else{
         $_SESSION['value_array'] = $_POST;
   ...
   function procDeleteComment(){
      ...
      /* Account edit successful */
      if($retval){
         header("Location: ".$session->referrer."?comment=3");
      }
      /* Error found with form */
      else{
         $_SESSION['value_array'] = $_POST;
   ...
   function procReportComment(){
      ...
      /* Account edit successful */
      if($retval){
         header("Location: ".$session->referrer."?comment=4");
      }
      /* Error found with form */
      else{
         $_SESSION['value_array'] = $_POST;
